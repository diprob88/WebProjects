...
?>	
	...
	while($row = mysql_fetch_array($result)) {
		...
		?>
		<option value='<?php echo $id;?>' <?php echo ($id==$prestazione_flt?'selected':'');?>><?php echo substr("$codice - $descrizione - &euro;$tariffa_euro", 0, 150);?></option>
	<?php	
    }
	?>
	</select>
...
function aggiorna_calendario() {
	...
	<tr>
		...
		for($j=1; $j<=7; $j++) {
			...
			// 	GIALLO se sul giorno insiste già almeno una prenotazione
			?>		
